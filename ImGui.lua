...
--// Customized: tuning block + optional title-bar focus effect

--===========================================================
--//  TUNING  —  edit these to restyle the UI
--===========================================================
local TUNING = {
	--// Title-bar color change on focus / unfocus
	--//   true  = title bar keeps ONE constant color (focus effect OFF)
	--//   false = blue when focused, dark when not (original behavior)
	NoSelectEffect = true,

	--// Set a Color3 to override, or nil to keep the prefab default
	TitleBarColor = Color3.fromRGB(32, 32, 36),   --// title bar background
	WindowColor   = Color3.fromRGB(20, 20, 22),   --// window / body background
	BorderColor   = Color3.fromRGB(0, 0, 0),      --// window outline
}
...
local ImGui = {
	...
	UIAssetId = "rbxassetid://85607317875581"
}

--// Expose tuning on the library
ImGui.Tuning = TUNING
...
function ImGui:ApplyTuning(Window, TitleBar)
	local T = ImGui.Tuning
	if not T then return end

	--// Title bar background
	if T.TitleBarColor and TitleBar then
		pcall(function() TitleBar.BackgroundColor3 = T.TitleBarColor end)
	end

	--// Window / body background
	if T.WindowColor then
		pcall(function() Window.BackgroundColor3 = T.WindowColor end)
		local Content = Window:FindFirstChild("Content")
		if Content then
			pcall(function() Content.BackgroundColor3 = T.WindowColor end)
			local Body = Content:FindFirstChild("Body")
			if Body then pcall(function() Body.BackgroundColor3 = T.WindowColor end) end
		end
	end
	...
	if T.BorderColor then
		...
		if Stroke then pcall(function() Stroke.Color = T.BorderColor end) end
	end
end
...
function ImGui:CreateWindow(WindowConfig)
	...
	--// Apply user tuning palette
	ImGui:ApplyTuning(Window, TitleBar)

	--// Window section events
	if not (WindowConfig.NoSelectEffect or (ImGui.Tuning and ImGui.Tuning.NoSelectEffect)) then
